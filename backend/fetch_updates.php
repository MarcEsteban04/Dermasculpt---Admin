...
$sql = "
    ...
    FROM (
        SELECT
            IF(m.sender_id = ? AND m.sender_role = 'dermatologist', m.receiver_id, m.sender_id) AS user_id,
            MAX(m.message_id) AS last_message_id
        FROM messages m
        WHERE (m.sender_id = ? AND m.sender_role = 'dermatologist') OR (m.receiver_id = ? AND m.receiver_role = 'dermatologist')
        GROUP BY user_id
    ) AS convos
